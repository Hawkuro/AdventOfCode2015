...
let isTheSue (sue : (string*int)[]) =
    let matchesTheSue (thing,number) = 
        match thing with 
        | "cats" | "trees" -> theSue.[thing] < number
        | "pomeranians" | "goldfish" -> theSue.[thing] > number
        | _ -> theSue.[thing] = number
    sue |> Array.map matchesTheSue |> Seq.reduce (&&)
